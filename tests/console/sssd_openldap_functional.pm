...
sub run {
    ...
    assert_script_run("curl -s " . data_url("sssd/openldap/add_test_repositories.sh") . " -o /tmp/add_test_repositories.sh");
    assert_script_run(qq(docker build -t openldap2_image --build-arg tag="$tag" --build-arg pkgs="$pkgs" --build-arg pkgs="$maint_test_repo" .), timeout => 600);
